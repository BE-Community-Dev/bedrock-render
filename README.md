...
and the current default palette. They are documentation samples, not full
BedrockMap parity output.
...
- `SurfaceBlocks` uses chunk bake: each chunk is reduced to a `16x16` terrain
  ...
  pixels.
- The BMCBL viewer uses `RenderLayout` auto-scaling. Small worlds default to
...
- The BMCBL GPUI map viewer starts from the world's `level.dat` spawn point,
  supports dimension/custom-dimension switching, and stores WebP web-map tiles
  under the system temporary cache directory.
...
- The GPUI viewer is a launcher preview window, not a full BedrockMap-compatible
  GIS application.
...
- V1 does not implement lighting, shadows, elevation blending, entity markers,
  labels, or full BedrockMap feature parity.
