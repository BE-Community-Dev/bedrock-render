...
- GPU compose 在单个 `wgpu` device 上使用有界并发队列；单个 tile 的 GPU 失败会
  fallback 到 CPU，并记录 `gpu_fallback_reason`。stats 也会暴露 GPU batch tile、
  submit/readback worker 和 buffer/staging pool 复用情况。
...
- Surface 渲染是 BedrockMap 风格顶视图，web-map 导出现在使用全局 chunk bake 队列和
  有界 GPU compose 队列；跨进程持久化 chunk bake 复用仍是后续优化项。
- V1 不实现实体标记或文字标签。
